...
def _clean_eprint(url):
    if "/scholar" == url[:8]:
        return _HOST + url.replace(";", "&")
    else:
        return url
    ...
    def __init__(self, __data, pubtype=None):
        ...
        if self.source == 'citations':
            ...
            if __data.find('div', class_='gs_ggs gs_fl'):
                # self.bib['eprint'] = [entr.a['href'] for entr in __data.find_all('div', class_='gs_ggs gs_fl')]
                # self.bib['eprint'] = __data.find('div', class_='gs_ggs gs_fl').a['href']
                self.bib['eprint'] = [_clean_eprint(entr.get("href")) for entr in __data.find('div', class_='gs_ggs gs_fl').find_all("a")]
        self._filled = False
    ...
    def fill(self):
        ...
        if self.source == 'citations':
            ...
            if soup.find('div', class_='gsc_vcd_title_ggi'):
                # self.bib['eprint'] = [entr.a['href'] for entr in soup.find_all('div', class_='gsc_vcd_title_ggi')]
                self.bib['eprint'] = [_clean_eprint(entr.get("href")) for entr in
                                      soup.find('div', class_='gsc_vcd_title_ggi').find_all("a")]
